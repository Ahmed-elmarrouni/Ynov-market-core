...
func Init() *gorm.DB {
	...
	// Detect if Postgres environment configuration exists natively
	...
	if dbUser != "" {
		...
		fmt.Println("Connecting to highly available PostgreSQL cluster on", dbHost)
		...
		fmt.Println("Falling back to local SQLite dependency...")
		dbPath := GetDBPath()

		// Ensure the directory exists
		if errDir := ensureDir(dbPath); errDir != nil {
			fmt.Println("db err: (Init - create dir) ", errDir)
		}

		db, err = gorm.Open(sqlite.Open(dbPath), &gorm.Config{})
		if err != nil {
			fmt.Println("db err: (Init SQLite) ", err)
		}
	}
	sqlDB, err := db.DB()
	if err != nil {
